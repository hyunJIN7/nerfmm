...
def store_current_pose(pose_net, pose_history_dir, epoch_i):
    ...
    for i in range(num_cams):
        ...
        c2w_list.append(c2w)
        # line = []
        # line.append(str(e) for e in torch.reshape(c2w,(-1,16))[:12].cpu().numpy())
        line = [str(e)for e in torch.reshape(c2w,(-1,16))[:12]]
        c2w_line_list.append(' '.join(line) + '\n')
    ...
    pose_file = os.path.join(pose_history_dir, str(epoch_i).zfill(6)+'.txt' )
    with open(pose_file, 'w') as f:
        f.writelines(c2w_line_list)
    return
